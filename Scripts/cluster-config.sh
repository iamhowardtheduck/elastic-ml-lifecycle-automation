...
echo '' >> /root/.env
echo 'ELASTICSEARCH_URL="http://kubernetes-vm:30920"' >> /root/.env
echo 'KIBANA_URL="http://kubernetes-vm:30002"' >> /root/.env
echo 'BUILD_NUMBER="10"' >> /root/.env
echo 'ELASTIC_VERSION="9.3.0"' >> /root/.env
...
curl -X POST "http://kubernetes-vm:30920/_security/user/sdg" -H "Content-Type: application/json" -u "elastic:${ELASTICSEARCH_PASSWORD}" -d '{
  "password" : "changeme",
  "roles" : [ "superuser" ],
  "full_name" : "SDG User",
...
#/opt/workshops/elastic-llm.sh -m gemini-2.5-pro -k false -d true

echo
#echo "Gemini 2.5 Pro AI Connector configured as openai-connector"
echo
echo 
echo
